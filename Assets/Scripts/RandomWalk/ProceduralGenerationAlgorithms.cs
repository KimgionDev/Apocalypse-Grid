...
public static class ProceduralGenerationAlgorithms
{
    ...
    {
        HashSet<Vector2Int>
            path = new HashSet<Vector2Int>(); // path chứa tất cả các vị trí đã đi qua trong quá trình random walk. HashSet được sử dụng để đảm bảo rằng mỗi vị trí chỉ được lưu trữ một lần, tránh trùng lặp.
    ...
    public static List<Vector2Int>
        RandomWalkCorridor(Vector2Int startPosition,
            int corridorLength) // Hàm này tạo ra một hành lang ngẫu nhiên bắt đầu từ startPosition và có độ dài corridorLength. Kết quả là một danh sách các vị trí sàn mới được tạo ra, đại diện cho hành lang.
    {
    ...
    */
    public static List<BoundsInt> BinarySpacePartioning(BoundsInt spaceToSplit, int minWidth, int minHeight)
    {
        ...
        List<BoundsInt>
            roomsList =
                new List<BoundsInt>(); // roomsList sẽ chứa tất cả các phòng đã được tạo ra sau khi quá trình chia không gian hoàn thành.
        Queue<BoundsInt>
            roomsQueue =
                new Queue<BoundsInt>(); // roomsQueue được sử dụng để quản lý các không gian cần được chia trong quá trình thực hiện thuật toán.
        ...
        while (roomsQueue.Count > 0)
        {
            ...
            if (room.size.y >= minHeight && room.size.x >= minWidth)
            {
                if (Random.value < 0.5f)
                {
                    if (room.size.y >=
                        minHeight *
                        2) // Nếu chiều cao của phòng đủ lớn để chia đôi theo chiều ngang, thực hiện chia theo chiều ngang
                    {
                        ...
                        (room.size.x >=
                         minWidth *
                         2) // Nếu chiều rộng của phòng đủ lớn để chia đôi theo chiều dọc, thực hiện chia theo chiều dọc
                    {
